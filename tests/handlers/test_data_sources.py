...
class TestDataSourceGetSchema(BaseTestCase):
    def test_fails_if_user_doesnt_belong_to_org(self):
        ...
        response = self.make_request(
            "get",
            f"/api/data_sources/{self.factory.data_source.id}/schema",
            user=other_user,
        )
...
class TestDataSourceResourceGet(BaseTestCase):
    def setUp(self):
        ...
        self.path = f"/api/data_sources/{self.factory.data_source.id}"
    ...
    def test_returns_only_view_only_for_users_without_list_permissions(self):
        ...
        rv = self.make_request("get", f"/api/data_sources/{data_source.id}", user=user)
        self.assertEqual(rv.status_code, 200)
...
class TestDataSourceResourceDelete(BaseTestCase):
    def test_deletes_the_data_source(self):
        ...
        rv = self.make_request(
            "delete", f"/api/data_sources/{data_source.id}", user=admin
        )
...
class TestDataSourcePausePost(BaseTestCase):
    def test_pauses_data_source(self):
        ...
        rv = self.make_request(
            "post",
            f"/api/data_sources/{self.factory.data_source.id}/pause",
            user=admin,
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_pause_sets_reason(self):
        ...
        rv = self.make_request(
            "post",
            f"/api/data_sources/{self.factory.data_source.id}/pause?reason=test",
            user=admin,
        )
        self.assertEqual(
    ...
    def test_requires_admin(self):
        rv = self.make_request(
            "post", f"/api/data_sources/{self.factory.data_source.id}/pause"
        )
        self.assertEqual(rv.status_code, 403)
...
class TestDataSourcePauseDelete(BaseTestCase):
    ...
    def test_requires_admin(self):
        rv = self.make_request(
            "delete", f"/api/data_sources/{self.factory.data_source.id}/pause"
        )
        self.assertEqual(rv.status_code, 403)
